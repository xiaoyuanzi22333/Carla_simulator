...
class World(object):
    ...
    def destroy_sensors(self):
        ...
        self.camera_depth.sensor.destroy()
        self.camera_depth.sensor = None
        self.camera_depth.index = None
        
        self.camera_RGB.sensor.destroy()
        self.camera_RGB.sensor = None
        self.camera_RGB.index = None
        
        self.camera_Lidar.sensor.destroy()
        self.camera_Lidar.sensor = None
        self.camera_Lidar.index = None
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # data preserve
                    ...
                    print("left brake pressed")
                    self.timer = time.time()
                    world.camera_depth.start_recorder()
                    world.camera_RGB.start_recorder()
                    world.radar_sensor.start_recorder()
                    world.gnss_sensor.start_recorder()
                elif event.key == K_RIGHTBRACKET:
                    print("right brake pressed")
                    world.camera_RGB.end_recorder()
                    world.camera_depth.end_recorder()
                    print("depth = " + str(len(depth_data_loader)))
                    print("RGB = " + str(len(RGB_data_loader)))
                    world.radar_sensor.end_recorder()
                    print("Radar data = " + str(len(Radar_data_loader)))
                    print("Radar timer = " + str(len(Radar_time_loader)))
                    world.gnss_sensor.end_recorder()
                    print("Gnss data = " + str(len(Gnss_data_loader)))
                    print("Gnss timer = " + str(len(Gnss_time_loader)))
                    print("time = " + str(time.time() - self.timer))
                    # for image in depth_data_loader:
                    #     image.save_to_disk('_out_depth/%08d' % image.frame)
                    # for image in RGB_data_loader:
                    #     image.save_to_disk('_out_RGB/%08d' % image.frame)
                    depth_data_loader.clear()
                    depth_time_loader.clear()
                    RGB_data_loader.clear()
                    RGB_time_loader.clear()
                    Radar_data_loader.clear()
                    Radar_time_loader.clear()
                    Gnss_data_loader.clear()
                    Gnss_time_loader.clear()
                    ...
                    world.hud.recorder = True
                    self.timer = time.time()
                elif event.key == K_k:
                    world.hud.recorder = False
                    ...
                    # pygame.image.save(surface_depth_loader[7], './img_test/depth_image'+str(0)+'.png')
                    # pygame.image.save(surface_RGB_loader[7], './img_test/RGB_image'+str(0)+'.png')
                    # pygame.image.save(surface_Lidar_loader[7], './img_test/Lidar_image'+str(0)+'.png')
                    
                    for idx,item in enumerate(surface_depth_loader):
                        pygame.image.save(item, './output_test/depth_image'+str(idx)+'.png')
                    for idx,item in enumerate(surface_RGB_loader):
                        pygame.image.save(item, './output_test/RGB_image'+str(idx)+'.png')
                    for idx,item in enumerate(surface_Lidar_loader):
                        pygame.image.save(item, './output_test/Lidar_image'+str(idx)+'.png')
                        
                    np.save("./output_test/Gnss_data.npy", client_Gnss_loader)
                    np.save("./output_test/imu_data.npy", client_imu_loader)
                    np.save("./output_test/time_data.npy", client_time_loader)
                    np.save("./output_test/radar_data.npy", client_radar_loader)
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if self.recorder:
            ...
            surface_depth_loader.append(world.camera_depth.surface)
            surface_Lidar_loader.append(world.camera_Lidar.surface)
            surface_RGB_loader.append(world.camera_RGB.surface)
            client_Gnss_loader.append((world.gnss_sensor.lat, world.gnss_sensor.lon))
            client_imu_loader.append((world.imu_sensor.accelerometer, world.imu_sensor.gyroscope))
            client_radar_loader.append(world.radar_sensor.radar_measure)
...
class GnssSensor(object):
    ...
    def _on_gnss_event(weak_self, event):
        ...
        if self.recorder:
            print(event)
            Gnss_data = {}
            Gnss_data['latitude'] = self.lat
            Gnss_data['longtitude'] = self.lon
            # Gnss_data['altitude'] = self.alt
            Gnss_data_loader[event.frame] = Gnss_data
            Gnss_time_loader[event.frame] = event.timestamp
